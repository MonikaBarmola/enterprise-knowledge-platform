...
A key feature of the platform is retrieval-level Role-Based Access Control (RBAC). Each user is assigned a role, and the system maps that role to the departments they are allowed to access. Documents are tagged with department metadata during ingestion, and unauthorized documents are filtered out during retrieval — before any context is sent to the LLM.

The application is built as an interactive Streamlit web application with authentication, source citations, and audit logging.
...
This permission mapping is applied during retrieval, not after — the department filter is passed directly into the vector search itself, so restricted content is excluded from the results before it ever reaches the LLM.
...
The current system uses K = 5, so Recall@K represents Recall@5.
...
```
The evaluation uses `test_questions.csv` and calculates Recall@K, Access-Control Accuracy, and Faithfulness.
